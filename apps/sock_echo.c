...
main (int argc, char *argv[]) {
    ...
    soc = socket_create(SOCKET_TYPE_UDP);
    if (soc == -1) {
        goto ERROR;
    }
    if (socket_bind(soc, NULL, hton16(ECHO_SERVER_PORT)) == -1) {
        goto ERROR;
    }
    fprintf(stderr, "waiting for messsage...\n");
    while (1) {
        len = socket_recvfrom(soc, buf, sizeof(buf), &peer_addr, &peer_port, -1);
        if (len <= 0) {
            break;
        }
        fprintf(stderr, "message form: %s:%u\n", ip_addr_ntop(&peer_addr, addr, sizeof(addr)), ntoh16(peer_port));
        hexdump(stderr, buf, len);
        socket_sendto(soc, buf, len, &peer_addr, peer_port);
    }
    socket_close(soc);
    cleanup();
    return  0;
